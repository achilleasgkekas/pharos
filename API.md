...
- `POST /api/v1/subscriptions` `{ name, amount, billingCycle?, startDate?, nextRenewal?, category?, provider?, url?, notes?, space?, trialEndsAt?, firstChargeAmount?, currency?, fxRate? }` → `{ subscription }`
...
- `PATCH /api/v1/subscriptions/:id` `{ name?, amount?, billingCycle?, nextRenewal?, category?, space?, active?, trialEndsAt?, firstChargeAmount?, currency?, fxRate? }` → `{ subscription }`
  - `space` is the optional per-property ledger tag (P68 phase 2), the same one Expenses and Receipts carry and the same taxonomy as Settings → Spaces; `''` = unassigned. Trimmed and capped at 40 chars. Only tagged subscriptions appear in the per-space monthly card in Reports, so leaving it empty changes nothing.
...
- `PATCH /api/v1/receipts/:id` `{ store?, date?, total?, subtotal?, vatAmount?, paymentMethod?, space?, notes?, verified?, archived?, lineItems?, currency?, fxRate? }` → `{ ok, receipt }`
  - `space` is the optional per-property ledger tag (P68), the same one Expenses carry and the same taxonomy as Settings → Spaces; `''` = unassigned. Trimmed and capped at 40 chars. Only tagged receipts join the per-space breakdown in Reports, so leaving it empty changes nothing.
  - Money fields are read as PRINTED figures (what the paper says). Sending any of them (or `currency`/`fxRate`) re-resolves the WHOLE receipt with one rate: total, net, VAT and every line price convert together, so a partial update can never leave it half-converted. Omitting `currency`/`fxRate` keeps the ones already stored, which is what a quick-verify (`store`/`date`/`total`/`verified`) relies on. `origAmount` is derived, never sent.
