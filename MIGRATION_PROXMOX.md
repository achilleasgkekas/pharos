...
2. ~~Επιβεβαίωσε ότι το nightly backup τρέχει και **από τη VM**, με άφιξη επαληθευμένη στο Storage
   ...
   Βρέθηκε επίσης ότι το backup προστάτευε τη ΛΑΘΟΣ εγκατάσταση (την άδεια φιλοξενούμενη, όχι το
   self-hosted όπου ζουν τα πραγματικά δεδομένα) και ότι δεν υπήρχε καν cron. Πλήρες ιστορικό στο
   `DEPLOY_LOG.md`, εγγραφές 28/8.
3. Σβήσε το VPS από το Hetzner Cloud.
4. ~~**Το Storage Box δεν το σβήνεις.**~~ **ΑΚΥΡΟ.** Το Storage Box δεν υπάρχει πλέον (βλ. 2).
   Ο offsite προορισμός είναι τώρα το NAS.
