...
/**
 * Parse the export query string. Delegates action/tenant/actor/from/to/cursor to the page's parser (identical
 ...
 * `pharos-audit-acme-member-added-2026-07-30.csv`, or with an actor and a window:
 ...
 * remember which slice it was — and that argument is strongest for the TIME window and the ACTOR,
 * since two exports of the same workspace and verb are otherwise indistinguishable by name. The
 * actor is prefixed `by-` so a mangled email (dots and `@` collapse to dashes) still reads as a
 * person rather than as another slug. The trailing date stays the day the file was generated,
 * which is a different fact from the window it covers. PURE.
 */
export function platformAuditCsvFilename(
  query: Pick<AdminAuditQuery, 'action' | 'tenant'> &
    Partial<Pick<AdminAuditQuery, 'actor' | 'from' | 'to'>>,
  now: Date = new Date()
): string {
  ...
  const actor = query.actor ? slugPart(query.actor, 'actor') : '';
  return (
    [
      'pharos-audit',
