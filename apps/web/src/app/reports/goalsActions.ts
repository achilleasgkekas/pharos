...
export async function createGoal(formData: FormData): Promise<{ ok: boolean; error?: string }> {
  ...
  await connectDB();
  await Goal.create({ ...raw, targetDate: safeDateOrNull(raw.targetDate), contributions: [], archived: false });
...
export async function updateGoal(id: string, formData: FormData): Promise<{ ok: boolean; error?: string }> {
  ...
  const raw = parsed.data;
  await connectDB();
  await Goal.findByIdAndUpdate(id, { ...raw, targetDate: safeDateOrNull(raw.targetDate) });
  revalidatePath('/reports');
  revalidatePath('/');
  return { ok: true };
}
