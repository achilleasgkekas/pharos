 ...
 * Returns the SAME shape as GET /api/v1/expenses (single `expense`) so the API client's
 * detail view can re-prefill in place. The re-scanned record is left unverified.
 */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withAuth(req, async () => {
    ...
    // Re-read with the exact GET serializer so the re-prefill matches the list shape.
    await connectDB();
