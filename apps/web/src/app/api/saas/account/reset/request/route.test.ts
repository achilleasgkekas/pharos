...
// ── abuse limit ─────────────────────────────────────────────────────────────────────────────
// This endpoint mails an address the CALLER picks, so unlimited it is an inbox-bombing tool that spends our sending reputation.
describe('rate limiting', () => {
  it('a limited caller gets the limiter response and NOTHING else runs', async () => {
    ...
    const res = await POST(makeReq({ email: 'someone@x.com' }));
  ...
  it('is keyed per client IP and per endpoint, so one endpoint cannot exhaust another', async () => {
    await POST(makeReq({ email: 'someone@x.com' }));

    expect(rateLimitMock).toHaveBeenCalledTimes(1);
    const key = rateLimitMock.mock.calls[0][0];
    expect(key).toContain('1.2.3.4');
    expect(key).toMatch(/^saas-reset-request:/);
  });
});
