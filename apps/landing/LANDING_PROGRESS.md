...
## 2026-08-03 (δ, μετρηση prod vs dev που ζητησε ο Αχιλλεας)

Ζητηθηκε να τραβηχτουν οι ιδιες 4 εικονες σε production build για να φανει ποσο πεφτει ο χρονος. **Το
αποτελεσμα αναιρει τη συσταση που ειχα γραψει στο προηγουμενο entry**: το prod δεν ειναι ουσιαστικα πιο
γρηγορο, ο ενοχος ηταν ο φορτος του μηχανηματος.

| σεναριο (ιδιες 4 εικονες, ιδια εντολη) | συνολο | chrome | warm | load | ληψεις |
|---|---|---|---|---|---|
| dev, μηχανημα σε load ~56 | **122s** | 9.2s | 22.0s | 33.9s | 2.8 / 0.34 / 1.7 / 0.85s |
| **prod** (`next start`), load ~17 | **4.4s** | 1.0s | 0.02s | 0.52s | 0.17 / 0.07 / 0.12 / 0.09s |
| dev, ιδιο ξεκουραστο μηχανημα | **4.2s** | 0.59s | 0.18s | 0.36s | 0.22 / 0.09 / 0.14 / 0.10s |
| dev με σβησμενο `.next` | **4.0s** | 0.74s | 0.13s | 0.40s | 0.17 / 0.05 / 0.12 / 0.07s |

**Το συμπερασμα**: prod και dev ειναι **πρακτικα ισοδυναμα** (4.4s vs 4.0-4.2s), και το prod χρειαζεται
επιπλεον **13s build**, οποτε βγαινει ελαφρως πιο ακριβο συνολικα. Τα 122s δεν ηταν κοστος του dev server:
το `uptime` τη στιγμη της μετρησης εδειχνε **load average 56.30 στο 15λεπτο** (Docker builds και αλλα
routines παραλληλα), και τοτε ενα σκετο `curl` στη σελιδα εκανε **50s**. Οταν το μηχανημα ηρεμησε (load 17),
ο ιδιος dev server σερβιρε σε **129-219ms**.

**Διορθωση της προηγουμενης συστασης**: ειχα γραψει «αν θελεις γρηγορες ληψεις, τραβα τες σε build+start».
Λαθος συμπερασμα απο μια μετρηση υπο φορτο. Το σωστο ειναι: **μεινε στο dev**, και αν καποτε δεις χρονους
σαν τα 122s, κοιτα το `uptime` πριν κατηγορησεις το setup. Το ενημερωσα και στο SKILL.md.

Δυο ακομα πραγματα που επιβεβαιωθηκαν στην πορεια:
- **prod και dev δινουν byte-identical εικονες**: το `md5` των τεσσαρων εκδοχων του `pricing-desktop.png`
  (ενα prod run + τρια dev runs) δινει **ενα** hash. Δηλαδη το dev δεν βαζει κατι δικο του στο rendering,
  αρα το visual proof απο dev ειναι το ιδιο εγκυρο.
- Το `preview_start` ανοιγει tab στο URL, οποτε **προλαβαινει να κανει compile** πριν τρεξει το script.
  Γι' αυτο ακομα και με σβησμενο `.next` το warm-up ηταν 129ms: το κοστος του πρωτου compile ειχε ηδη
  πληρωθει απο το tab. Καλο να το ξερει οποιος διαβασει τους αριθμους παραπανω.
