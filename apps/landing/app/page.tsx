...
const ROADMAP: {
...
}[] = [
  {
    ...
    items: [
      ...
      'Opt-in multi-currency for expenses, income, receipts, subscriptions, items, statements & CSV imports',
    ],
  },
  {
    phase: 'Building',
...
const FAQS: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: 'Yes, opt-in. Turn on multi-currency in Settings and the expense, income, receipt, subscription, item, and statement forms grow a currency picker plus an exchange-rate field: enter the printed foreign amount and either the rate or what your card actually got charged, and PHAROS backs the rate out for you. On a receipt every amount converts with that same rate, not just the total, since reports sum VAT and the item library copies line prices into your inventory, so a half-converted receipt would throw both off. A foreign-currency subscription converts its recurring charge and its post-trial first-charge amount with that same rate too, so a "cancel before you get charged" reminder and the monthly/yearly totals never mix currencies inside one record. An item bought abroad converts its purchased, current, and target prices together, so net worth and the insurance export see one consistent figure instead of a euro number quietly standing in for dollars. A card statement converts as a whole document too: the total, minimum payment, and every individual charge, since installment payoff figures are summed from those same charges. Reports, budgets, and net worth keep summing everything in your base currency underneath, so nothing else changes. Leave a rate unset and a gold badge flags it rather than silently guessing a 1:1 conversion, and Reports keeps a running list of every record still missing one, largest printed amount first, each row linking straight to that record so a CSV import that left a dozen unrated rows behind does not send you hunting for gold badges one page at a time. It now covers every money-holding record in the app and every way it gets in: expenses, income, receipts, subscriptions, items, and statements typed in or scanned, plus foreign-currency rows in a bank CSV import.',
  },
  {
