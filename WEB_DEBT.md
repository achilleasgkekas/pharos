...
- Status: TODO (flagged 2026-07-03 reviewer, range 148fb06..53b861a· 38η σάρωση επιβεβαίωσε ανοιχτό: `grep tenant.status src/lib/tenancy src/lib/apiAuth.ts` = μόνο projection hits, μηδέν gate στο v1 data path· ο `workspace` DELETE θέτει `status:'canceled'` που κανείς δεν επιβάλλει στο `getTenantContext`. UPDATE: το reactivate-blocker λύθηκε — νέο `POST /api/saas/workspace/reactivate` (`bbb09d1`) δίνει owner-only δρόμο επιστροφής, άρα το gating του v1 path είναι πλέον ασφαλές να υλοποιηθεί· απομένει μόνο το read-vs-write product decision)
