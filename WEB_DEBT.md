...
- Acceptance:
  ...
  - **Το χάσμα μεγαλώνει σταθερά, ίδιο-scoped commits το κάνουν σκόπιμα (documented precedent, δες PROGRESS.md 2026-07-15/07-16/07-19 runs)**: 38 (2026-07-10) → 42 (53η σάρωση, 2026-07-15, +4 P7) → 75 (54η σάρωση, 2026-07-18, +33) → **84 (reviewer, 2026-07-19, +9)**. Τα +9 της τελευταίας περιόδου, όλα από commits `536a3d8`/`6ed76b6` σε αυτό το review range (`82e008c..638e33a`): `trash.tLoyaltyCard` (P20 loyalty-card wallet, 1 κλειδί)· `migrate.sectionTitle`/`migrate.sectionHint`/`migrate.ynabButton`/`ynab.title`/`ynab.dropHint`/`ynab.formats`/`ynab.notYnab`/`ynab.summary` (P16 YNAB CSV migration importer, 8 κλειδιά). ΣΗΜ: πέρα από τα λείποντα `el.ts` keys, το νέο `LoyaltyCardsClient.tsx` (P20) δεν περνάει καν από `t()` (μόλις 2 references) — mirror του ήδη-γνωστού `GiftCardsClient.tsx` gap (3 references), δηλαδή το UI του tab δεν είναι απλά αμετάφραστο στα ελληνικά, είναι ΚΑΙ μη-i18n-wired hardcoded strings· δεν το πρόσθεσα ως ξεχωριστό item (ίδιας κλάσης precedent με το ήδη-γνωστό gift-card gap, όχι νέο pattern).
  - **Πλήρης λίστα (comm en−el, 84 live 2026-07-19):** `nav.bills`, `home.dBills`, `home.dGoals`, `home.onbTitle`, `home.onbSubtitle`, `home.onbDone`, `home.onbStorage`, `home.onbReceipt`, `home.onbBudget`, `home.onbCard`, `home.onbNotify`, `trash.tGiftCard`, `trash.tLoyaltyCard`, `trash.tBill`, `trash.tGoal`, `reports.cExpBySpace`, `reports.cGoals`, `reports.gNewGoal`, `reports.gNoGoals`, `reports.gTitle`, `reports.gTitlePlaceholder`, `reports.gTarget`, `reports.gDeadline`, `reports.gReached`, `reports.gPerMonth`, `reports.gAddAmount`, `reports.gDeleteTitle`, `reports.gDeleteBody`, `sub.discoveredTitle`, `sub.discoveredOccurrences`, `sub.discoveredTrack`, `sub.discoveredDismiss`, `ex.space`, `ex.fSpace`, `ex.allSpaces`, `ex.spaceNone`, `ex.splitTitle`, `ex.splitEmpty`, `ex.splitName`, `ex.splitAddPerson`, `ex.splitEqually`, `ex.splitIncludeMe`, `ex.splitMarkPaid`, `ex.splitOwedYou`, `ex.splitSettled`, `ex.splitYourShare`, `ex.balancesTitle`, `ex.balancesBtn`, `ex.balancesEmpty`, `ex.balancesSettled`, `ex.balanceEntries`, `ex.settleUp`, `ex.settleTitle`, `ex.settleBody`, `set.giftCardAlert`, `set.billAlert`, `set.spaces`, `set.spacesDesc`, `set.spacesEmpty`, `set.spacesPlaceholder`, `notif.giftcardSub`, `notif.giftcardTodaySub`, `notif.billDueSub`, `notif.billTodaySub`, `notif.billOverdueSub`, `set.sampleData`, `set.sampleDataDesc`, `set.sampleLoad`, `set.sampleReload`, `set.sampleClear`, `set.sampleReloadTitle`, `set.sampleReloadConfirm`, `set.sampleClearTitle`, `set.sampleClearConfirm`, `set.sampleLoaded`, `set.sampleCleared`, `migrate.sectionTitle`, `migrate.sectionHint`, `migrate.ynabButton`, `ynab.title`, `ynab.dropHint`, `ynab.formats`, `ynab.notYnab`, `ynab.summary`.
  ...
  - **ΣΗΜ (γιατί flag, όχι fix από reviewer):** 84 μεταφράσεις είναι judgment call ακριβείας (el = primary γλώσσα του χρήστη), όχι μηχανικό one-liner· ανήκει στον builder, ιδανικά σε ένα αφιερωμένο μεταφραστικό pass (το χάσμα θα συνεχίσει να μεγαλώνει ανά feature αν δεν γίνει). Μη-blocking (English fallback ενεργό).
...
- Status: TODO (flagged 2026-07-10 reviewer· live: en=1179 keys, el=1141, 38 missing από P28/P32/P34/P35· 53η σάρωση 2026-07-15: 42 missing (+4 P7)· 54η σάρωση 2026-07-18: 75 missing (+33, από P12/P26/P1)· **reviewer 2026-07-19: 84 missing (+9, από P20 loyalty-card + P16 YNAB import)** — gap μεγαλώνει σταθερά, αξίζει προτεραιοποίηση πριν φτάσει σε δύσκολο-να-καλυφθεί μέγεθος)
