...
## 2026-08-04 (thirty-ninth run — AI assistant / search now cover all 12 modules)

Σάρωση git log για νέα feat() commits μετά την thirty-eighth run (64943cf, 2026-08-03). Βρέθηκαν αρκετά νέα feat() commits: **d774dd4** (feat(ai): let the assistant see and act on every module, not three of twelve), **f123e94** (feat(landing): drop the permanent free hosted tier, hosted now opens on a 14-day trial), **6c5b021** (feat(saas): local multi-tenant test stack + cross-subdomain session cookie), **fda8c96** (feat(alerts): schedulable alert sweep endpoint, and unblock it in middleware), **60d25f4** (feat(backup): check a backup before you need it, and stop restoring rubbish quietly), plus fixes **2d21247**/**56e983e**. Διάλεξα το παλαιότερο (d774dd4) — user-facing, core-territory, όχι SaaS/landing/dev-tooling.

Τι έγινε: Διάβασα τον κώδικα πριν γράψω, όχι το commit message — `apps/web/src/app/search-actions.ts` (searchAll πλέον κάνει query σε **12** collections: items/receipts/statements/tasks/subscriptions/expenses/vouchers + **bills/goals/giftcards/loyaltycards/shoppinglist** νέα, καθεμιά με δικό της href pattern· goals→`/reports#goals` anchor, shoppinglist→`/shopping-list` flat, giftcard/loyaltycard→`/vouchers?tab=X&open=id`, γιατί δεν έχουν ?open= consumer ή ζουν σε tabs) + `apps/web/src/app/aiTools.ts` (`EDITABLE_MODELS` πλέον **10** τύποι, όχι 3· `update_record`/`delete_record` δουλεύουν σε item/task/subscription/expense/voucher/bill/goal/giftcard/loyaltycard/shoppinglist· **2 σκόπιμα carve-outs**: statement δεν έχει soft-delete plugin [unique {card,period} index θα μπλόκαρε re-import ενώ trashed copy κρατά το slot] άρα delete θα ήταν μη-αναστρέψιμο ενώ το tool υπόσχεται 30-day Trash recovery — ψέμα αν το επέτρεπε· receipt ίδιο σκεπτικό [scanned doc με file+lineItems+installment links]· και τα δύο μένουν **findable** μόνο· gift card `uses` + goal `contributions` = derived-balance ledgers, screenFields τα αρνείται ρητά [error, όχι silent drop] γιατί ένα LLM rewrite θα αλλοίωνε το balance σιωπηλά).

Ενημερώθηκε το `docs/features.md`: (α) το section **"AI command bar & history"** επεκτάθηκε με shopping-list lines στα add-able + νέα παράγραφος που εξηγεί τα δύο deliberate gaps (statement/receipt findable-not-editable + gift card/goal ledger fields refused-not-dropped)· (β) το section **"Search"** ενημερώθηκε με τα 5 νέα searchable modules (bills/goals/gift cards/loyalty cards/shopping list) + νότα ότι το deep-link πάει σε section-anchor όταν δεν υπάρχει per-row detail view (goals/shopping-list).

Accuracy: verified με code read (search-actions.ts πλήρες searchAll σώμα + όλα τα hit-builders, aiTools.ts EDITABLE_MODELS/BLOCKED_FIELDS/screenFields πλήρες σώμα + inline σχόλια του ίδιου του commit που εξηγούν το reasoning), όχι απλή αντιγραφή commit message. Έλεγξα ότι όλα τα referenced anchors/routes υπάρχουν ήδη στο `docs/features.md` (`#shopping-list`, `#goals` μέσω `/reports#goals`).

Validation (markdown only, κανένα build/Docker/AI call): κανένα code fence στο `docs/features.md` (0 πριν/μετά, το αρχείο δεν έχει code blocks)· κανένα νέο anchor δημιουργήθηκε (edits μέσα στα ήδη-υπάρχοντα "AI command bar & history"/"Search" sections)· `docs/api.md` ελέγχθηκε (`/ai` endpoint row + MCP tools/list section) — ήδη γενικόληπτο ("add / update / search records"), δεν χρειαζόταν αλλαγή.

Collision guard: `git status --short -- docs/` πριν commit = μόνο `docs/features.md` modified (δικό μου), κανένα foreign staged.

Δεν έγινε (out of scope αυτό το run, θα καλυφθούν σε επόμενα): f123e94 (landing pricing — `docs/saas.md` ήδη σωστά αναφέρει plans.ts ως single source of truth, χρειάζεται μόνο επιβεβαίωση ότι δεν αναφέρει "permanent free tier" πουθενά)· fda8c96 (alerts scheduler endpoint — πιθανό OpenAPI/api.md gap)· 60d25f4 (backup verification — `docs/backup-and-restore.md` update)· 6c5b021 (local SaaS test stack — dev-tooling, μάλλον όχι user-facing docs territory).

Συμπέρασμα: Το shipped feature d774dd4 (AI assistant + search σε όλα τα 12 modules) είναι πλέον fully + accurately documented στο `docs/features.md`.

Επόμενο run: (α) f123e94 landing pricing claim check στο `docs/saas.md` · (β) fda8c96 alerts scheduler endpoint στο `docs/api.md`/openapi.yaml αν λείπει · (γ) 60d25f4 backup verification behavior στο `docs/backup-and-restore.md` · (δ) grep git log για νέα feat() commits μετά το 60d25f4.

## 2026-08-04 (fortieth run — cron alert-sweep endpoint documented in api.md)

Συνέχεια από την προηγούμενη in-conversation εντολή του χρήστη ("do the next item, alerts scheduler endpoint") πάνω στην ουρά της thirty-ninth run: **fda8c96** (`feat(alerts): schedulable alert sweep endpoint, and unblock it in middleware`, P81).

Διάγνωση πριν το γράψιμο: το `docs/self-hosting.md` είχε **ήδη** ενημερωθεί μέσα στο ίδιο commit fda8c96 (ο δημιουργός του feature το πρόσθεσε ο ίδιος — section "Scheduling the alert sweep" με crontab recipe, CRON_SECRET env var + το σωστό table entry) → verified section-by-section, ήταν ήδη πλήρες και ακριβές, καμία αλλαγή χρειάστηκε εκεί. Αυτό που έλειπε ήταν η **API reference εγγραφή** στο `docs/api.md` (νέο endpoint, κανένα entry στο "Other endpoints" section όπου ζουν το calendar feed + το MCP server).

⚠ **Σημαντικό εύρημα**: το working tree είχε ΗΔΗ uncommitted modifications σε `apps/web/src/app/api/cron/alerts/route.ts` + `route.test.ts` (ορατό στο αρχικό `git status` της συνεδρίας) — μια ενεργή WIP αλλαγή από άλλη routine που καλωδιώνει `runAlertChecks({dedupe:true})` (P82, alert dedup) μέσα στο ίδιο route. Το commit `2e293ca` («commit the abandoned alert-dedup work sitting in the tree») είχε ήδη κάνει commit το `lib/alertDedup.ts` + `settings/actions.ts` dedupe machinery, αλλά ΟΧΙ αυτά τα δύο route files — έμειναν ξανά uncommitted. **Documentation βασίστηκε ΑΠΟΚΛΕΙΣΤΙΚΑ στο committed state (fda8c96 στο main)**, ΟΧΙ στο working tree: η νέα εγγραφή περιγράφει `runAlertChecks()` χωρίς dedupe, όπως είναι πράγματι στο main αυτή τη στιγμή. Τεκμηρίωση unshipped/uncommitted κώδικα θα ήταν λάθος τη στιγμή που committed (μπορεί να αλλάξει ή να εγκαταλειφθεί, όπως ακριβώς συνέβη μια φορά ήδη με αυτό το ίδιο dedupe patch).

Τι έγινε: Διάβασα `apps/web/src/app/api/cron/alerts/route.ts` (committed, `git show fda8c96:...` semantics = τρέχον main HEAD μιας και δεν έχει committed αλλαγές μετά) + `lib/cronAuth.ts` (`checkCronAuth`: constant-time compare, fail-closed 500 όταν λείπει το `CRON_SECRET`, 401 σε missing/wrong token) + `middleware.ts` (matcher regex αποκλείει explicit `api/cron` από το session gate, με comment που εξηγεί γιατί: route tests καλούν `POST()` απευθείας και προσπερνούν το middleware, γι' αυτό χρειάστηκε ξεχωριστό `middleware.matcher.test.ts`). Νέο **"### Cron: alert sweep"** subsection στο `docs/api.md` (μετά το MCP section, πριν το "Quick start"): method/path table, self-host-only 404 υπό SAAS_MODE + το γιατί (shared DB χωρίς tenant scoping), CRON_SECRET bearer auth (fail-closed 500 vs 401), τα δύο response shapes (200 ok/sent/summary, 500 ok:false/error), curl παράδειγμα, cross-link στο ήδη-υπάρχον `self-hosting.md#scheduling-the-alert-sweep`. Ενημερώθηκε και η εισαγωγική παράγραφος του "Other endpoints" section (πρόσθεσε τρίτο λόγο: scheduler caller με shared-secret auth αντί per-user token).

Accuracy: verified με code read (route.ts committed session πλήρες σώμα + cronAuth.ts πλήρες + middleware.ts matcher comment), ΟΧΙ commit message copy. Explicit exclusion του uncommitted dedupe wiring με τεκμηρίωση του γιατί.

Validation (markdown only): code fences `docs/api.md` = 24 πριν/μετά (ζυγό — 1 νέο bash block προστέθηκε, αλλά η μέτρηση ήταν ήδη ζυγή μετά την προσθήκη, verified με count)· το cross-link anchor `self-hosting.md#scheduling-the-alert-sweep` επιβεβαιώθηκε ότι υπάρχει (heading "## Scheduling the alert sweep" ήδη στο αρχείο)· κανένα credential (μόνο `$CRON_SECRET` ως shell var, ίδιο pattern με το self-hosting.md παράδειγμα).

Collision guard: `git status --short -- docs/` πριν commit = μόνο `docs/api.md` modified (δικό μου), κανένα foreign staged. Το uncommitted `route.ts`/`route.test.ts` (εκτός docs/ territory) αφέθηκε εντελώς άθικτο.

Συμπέρασμα: Το shipped feature fda8c96 (cron alert-sweep endpoint) είναι πλέον fully documented και στα δύο μέρη όπου ανήκει: `docs/self-hosting.md` (operator how-to, ήδη έτοιμο από το ίδιο commit) + `docs/api.md` (API reference, προστέθηκε τώρα).

Επόμενο run: (α) f123e94 landing pricing claim check στο `docs/saas.md` · (β) 60d25f4 backup verification behavior στο `docs/backup-and-restore.md` · (γ) grep git log για νέα feat() commits μετά το πιο πρόσφατο εδώ (fda8c96, ήδη έγιναν αρκετά commits μετά — δες την προηγούμενη run entry) · (δ) αν route.ts/route.test.ts dedupe wiring committed μέχρι τότε, ενημέρωσε το `docs/api.md`/`self-hosting.md` cron section με το dedupe behavior.

## 2026-08-04 (forty-first run — backup verification behavior documented)

Συνέχεια από την ουρά της thirty-ninth/fortieth run κατόπιν ρητής in-conversation εντολής του χρήστη ("do the next item, backup verification behavior"): **60d25f4** (`feat(backup): check a backup before you need it, and stop restoring rubbish quietly`, app-level JSON backup verify) + **56e983e** (`fix(backup): verify the dump instead of trusting that a file appeared`, το nightly `scripts/backup.sh`). Και οι δύο αφορούν το ίδιο ριζικό πρόβλημα (ένα backup που "υπάρχει" ≠ ένα backup που δουλεύει) σε δύο διαφορετικά επίπεδα, οπότε τεκμηριώθηκαν μαζί στο `docs/backup-and-restore.md`.

Διάβασα τον κώδικα πριν γράψω, όχι τα commit messages:
- **`scripts/backup.sh`** (πλήρες, μετά το rewrite): credentials πλέον διαβάζονται ΜΕΣΑ από το container (`docker exec` reads `$MONGO_INITDB_ROOT_USERNAME`/`PASSWORD`) — τα παλιά `MONGO_USER`/`MONGO_PASS` env vars **δεν διαβάζονται πια καθόλου**, το table στο doc τα είχε ακόμα (stale, αφαιρέθηκαν)· dump→`.partial` πρώτα, μόνο μετά από verify γίνεται `mv` στο τελικό filename· verify = parse των `mongodump` stderr lines (collection/doc counts) + πραγματικό `mongorestore --dryRun` μέσα στο container (διαβάζει/parse-άρει ολόκληρο το archive χωρίς να γράφει τίποτα)· failure = `LAST_RUN_FAILED.txt` marker (καθαρίζεται στο επόμενο success) + ntfy ping (διαβάζει live το ήδη-configured ntfy από το `appconfigs` collection μέσα στο container, όχι δικό του env)· retention πλέον ΠΟΤΕ δεν σβήνει το τρέχον newest valid archive.
- **`scripts/restore.sh`** (ΔΕΝ άλλαξε σε αυτά τα commits, αλλά το doc το συνέδεε λανθασμένα με το backup.sh): ακόμα διαβάζει `MONGO_USER`/`MONGO_PASS` δικά του — η γραμμή «Same MONGO_* env vars as backup.sh» έγινε **λάθος** μετά το backup.sh rewrite (το backup.sh πια δεν τα διαβάζει), diορθώθηκε ρητά.
- **`lib/backupModels.ts`** (`BACKUP_MODELS`): η λίστα των collections στο JSON export/import είναι **15**, όχι 8 όπως έλεγε το doc — **σημαντικό ξεχωριστό εύρημα, εκτός του immediate commit scope αλλά flat-out λάθος**: το doc έλεγε ρητά "What it does NOT include: Expenses / income records", ενώ το `expenses` key (kind income/expense) μπήκε στο backup registry ήδη από τις 2026-07-26 (σχόλιο μέσα στο ίδιο αρχείο, commit άσχετο με το σημερινό task) μαζί με bills/goals/giftCards/loyaltyCards/netWorthSnapshots/shoppingList. Διορθώθηκε η πλήρης λίστα + η exclusion λίστα ευθυγραμμίστηκε με το `BACKUP_EXCLUDED` registry (AppConfig/User/Account/transient state), αντί για το γενικόλογο "App settings, budgets, AI/storage/notification configuration" που δεν ανέφερε καν logins.
- **`lib/backupVerify.ts`** (`verifyBackupJson`, πλήρες σώμα) + **`settings/actions.ts`** (`verifyBackup`/`importData`, πλήρες σώμα) + **`SettingsClient.tsx`** (`BackupRestore`/`handleVerify`, πλήρες σώμα): το νέο **error/warning** two-level σχήμα (error = μη-χρησιμοποιήσιμο αρχείο συνολικά → `importData` αρνείται εντελώς αντί για half-restore· warning = κομμάτια θα παραλειφθούν αλλά η υπόλοιπη restore προχωράει) + το UI (`Verify…` κουμπί δίπλα στο Export/Restore, file picker, read-only, δείχνει exportedAt + `formatBackupCounts` human summary ή τη λίστα issues).

Ενημερώθηκε το `docs/backup-and-restore.md`: (α) section 1 "`Database — scripts/backup.sh`" ξαναγράφτηκε πλήρως — αφαιρέθηκαν MONGO_USER/MONGO_PASS από το table, προστέθηκαν MIN_BYTES/NTFY_URL, νέα παράγραφος εξηγεί το `.partial`→verify→move flow, το `--dryRun` mechanism, το failure marker+ntfy, και το "ποτέ μη σβήνεις το newest" retention rule· (β) section 3 "Restoring from a dump" διόρθωσε τη λανθασμένη "Same MONGO_* env vars as backup.sh" αναφορά· (γ) section 4 "In-app JSON export/import" — διορθώθηκε η λίστα από 8→15 collections (με σημείωση ότι income ζει μέσα στο `expenses` key), η exclusion λίστα ευθυγραμμίστηκε με το πραγματικό BACKUP_EXCLUDED (πρόσθεσε ρητά logins που έλειπαν εντελώς), και νέο subsection **"### Verify a backup before you need it"** που εξηγεί το read-only check, τα δύο issue levels, και ότι το Restore τρέχει το ίδιο check πρώτα (refuse σε error, report-and-proceed σε warning, no-`_id` docs εισάγονται ως νέα αντί για merge).

Accuracy: verified με code read (backup.sh πλήρες μετά το rewrite comment block + πλήρες script σώμα, backupVerify.ts πλήρες, backupModels.ts πλήρες registry+exclusions, settings/actions.ts verifyBackup/importData πλήρη σώματα, SettingsClient.tsx BackupRestore component), όχι απλή αντιγραφή commit message. Η ασυμφωνία στη λίστα collections (8 vs 15) βρέθηκε επειδή διάβασα το πραγματικό `backupModels.ts` αντί να εμπιστευτώ το ήδη-υπάρχον doc text.

Validation (markdown only, κανένα build/Docker/AI call): code fences `docs/backup-and-restore.md` = 8 πριν και μετά (ζυγό, άθικτο)· κανένα νέο top-level anchor δημιουργήθηκε στο TOC (το νέο "### Verify a backup before you need it" είναι h3 subsection, ίδιο πάτερν με το ήδη-υπάρχον "### Database — scripts/backup.sh" — το TOC λιστάρει μόνο h2)· η cross-reference "section 1"/"section 4" παραμένουν σωστές (δεν άλλαξε η αρίθμηση)· κανένα credential.

Collision guard: `git status --short -- docs/` πριν commit = μόνο `docs/backup-and-restore.md` modified (δικό μου), κανένα foreign staged.

Δεν έγινε (out of scope αυτό το run): `exportInsuranceBundle`/`exportTaxBundle` (νέα ZIP exports που βρέθηκαν στο ίδιο `SettingsClient.tsx` αλλά ΔΕΝ σχετίζονται με τα σημερινά δύο commits — δεν φαίνεται να είναι τεκμηριωμένα πουθενά στο `docs/`, πιθανό ξεχωριστό gap για επόμενο run)· f123e94 landing pricing claim check.
...
## 2026-08-09 (forty-fifth run — saas.md MFA login-wiring staleness fixed, mailer env vars corrected)

Έλεγξα πρώτα το ask-inbox (`grep pharos-docs ~/.claude/ASK_ACHILLEAS.md`) — καμία εγγραφή. `docs/` καθαρό στο αρχικό `git status --short` (μόνο `.vite/` untracked, εκτός territory).

Η ουρά της forty-fourth run έλεγε "item 3: `4b87112` real SMTP email send". Ξεκίνησα διαβάζοντας τον κώδικα (`apps/web/src/lib/tenancy/mailer.ts`) για να τεκμηριώσω το SMTP, αλλά ενώ διάβαζα το ήδη-υπάρχον `docs/saas.md` section "Multi-factor authentication (MFA)" για context (γειτονικό section, mailer-related dev-scaffold note ζει εκεί), βρήκα κάτι πιο σοβαρό: **μια self-contradictory stale claim μέσα στο ίδιο αρχείο**. Η γραμμή "**Login integration is not yet wired** (increment 80c, separate)" (implementation note, γραμμή ~311) αντιφάσκει ρητά με την ήδη-υπάρχουσα ενότητα "Rate limiting" λίγες γραμμές πιο πάνω (γραμμή 190, `POST /api/saas/auth/mfa (second-factor verification) — rate limited per pending account`) — δεν μπορεί να είναι "not wired" ένα endpoint που το ίδιο doc ήδη περιγράφει ως rate-limited σε λειτουργία. Verified με code read (όχι commit message μαντεψιά): `git log -- mfaStore.ts` έδειξε **`96e5cbb` feat(saas): MFA login-flow wiring (increment 83, TODO §9)`, 2026-07-21** — 2 εβδομάδες πριν από αυτό το run, ήδη merged. Διάβασα πλήρη σώματα: `apps/web/src/app/api/saas/auth/login/route.ts` (αν `account.mfaEnabled` → `setMfaPendingCookie` + `{mfaRequired:true}`, καμία session ακόμα), `apps/web/src/app/api/saas/auth/mfa/route.ts` (POST verify code/recovery→session, DELETE cancel pending), `apps/web/src/components/saas/AuthForm.tsx` (πλήρες two-step client flow, `step: 'credentials'|'mfa'`).

Και ένα δεύτερο εύρημα στο ίδιο section: η καταληκτική γραμμή "Self-hosted app has no account login / multi-tenant features; MFA is out of scope" είναι πλέον **λάθος στην ουσία** μετά το `ed68936` (documented στο forty-third run) — το self-hosted app ΕΧΕΙ πλέον TOTP 2FA (δικό του, ξεχωριστό primitive πάνω στο `User` model, όχι στο SaaS `Account`). Η φράση παραπλανεί έναν αναγνώστη να νομίζει ότι self-host = μηδέν MFA.

Fixes στο `docs/saas.md` (5 σημεία, ίδιο section + ένα γειτονικό):
1. **Authentication table**: το `POST /api/saas/auth/login` row πήρε ρητή περιγραφή του `mfaRequired:true` branch· προστέθηκαν 2 νέες γραμμές `POST`/`DELETE /api/saas/auth/mfa` (login step 2 + cancel) που δεν είχαν ποτέ δικό τους table row, μόνο μια πλάγια αναφορά στο rate-limiting section.
2. **Browser sign-in UI table**: το `/account/login` row ξαναγράφτηκε ως two-step flow (credentials → αν `mfaRequired` code-entry step → `POST .../mfa` → navigate), matched στο πραγματικό `AuthForm.tsx` state machine.
3. **Implementation note**: "not yet wired" → "fully wired" με ακριβή περιγραφή του τι αλλάζει στο login response.
4. **Closing "SaaS-only" γραμμή**: διορθώθηκε ώστε να μην αντιφάσκει με το self-host TOTP· cross-link στο `docs/security.md#1-session-cookie-the-web-ui` (verified anchor υπάρχει, exact heading text match).
5. **Dev-scaffold note** (γειτονικό, ίδιο mailer-adjacent context): "until a mailer is wired up" → "when no mailer provider is configured at all" (η ίδια ακρίβεια-ζήτημα λογική όπως το MFA fix — mailer ΕΙΝΑΙ wired πλέον, three providers).

Στην πορεία διάβασα και το `SaaS environment variables` table (mailer-adjacent, το item 3 του queue) και βρήκα ότι είναι **ξεπερασμένο σε βάθος**, όχι απλά stale-ημερομηνία: είχε μόνο `SMTP_URL` (η αρχική connection-string προσέγγιση από το `4b87112`), αλλά ο πραγματικός deployed κώδικας (`92e5aec` fix commit, μετά το `4b87112`) χρησιμοποιεί **discrete `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS`** fields (verified με `deploy/.env.prod.example`, το πραγματικό production config του χρήστη — Gmail App Password setup) — και έλειπαν εντελώς τα `MAIL_WEBHOOK_URL`/`MAIL_WEBHOOK_TOKEN` (middle-precedence provider, `4ec7af4`). Ξαναγράφτηκε το table entry πλήρες: 3 mailer providers με precedence order (Resend > webhook > SMTP), γιατί τα discrete SMTP fields προτιμώνται (το `@` του email username σπάει το URL parsing), Gmail App Password specifics (2-Step Verification, strip spaces, `MAIL_FROM` πρέπει να ταιριάζει το authenticated mailbox γιατί το Gmail rewrites το From header, ~500/day free ceiling), και ρητή σημείωση ότι το παλιό `SMTP_URL` string διαβάζεται ακόμα ως fallback.

Accuracy: verified με πλήρη code read (login/route.ts, auth/mfa/route.ts, AuthForm.tsx, mailer.ts πλήρες σώμα μετά όλα τα follow-up commits `92e5aec`/`3161efd`/`846e5be`/`8332b12`, `deploy/.env.prod.example`), όχι commit-message copy. Και τα δύο staleness ευρήματα (MFA "not wired", "SMTP_URL only") βρέθηκαν επειδή διάβασα τον τρέχοντα κώδικα δίπλα στο ήδη-υπάρχον doc text αντί να το εμπιστευτώ.

Validation (markdown only, κανένα build/Docker/AI call): code fences `docs/saas.md` = 18 πριν/μετά (ζυγό, καμία αλλαγή σε code block)· anchors verified ρητά (`#authentication`, `#browser-sign-in-ui-accountlogin-accountsignup`, `#multi-factor-authentication-mfa`, `#saas-environment-variables`, `docs/security.md#1-session-cookie-the-web-ui` — όλα βρέθηκαν ως πραγματικά headings με exact-text match)· κανένα credential/secret literal (το Gmail παράδειγμα είναι το ίδιο placeholder pattern που ήδη υπάρχει μέσα στο `deploy/.env.prod.example`, με κενό `SMTP_PASS`).

Collision guard: `git status --short -- docs/` πριν commit = μόνο `docs/saas.md` modified (δικό μου), κανένα foreign staged.

Δεν έγινε (out of scope, θα το πιάσει το επόμενο run αν χρειάζεται νέο υλικό πέρα από τη διόρθωση): τεκμηρίωση του `nodemailer` ως runtime dependency στο `docs/self-hosting.md` (build-time concern, μάλλον ήδη καλυμμένο έμμεσα από το γενικό "Docker image includes all dependencies" — δεν το έλεγξα ρητά αυτό το run).

Συμπέρασμα: Το `docs/saas.md` MFA section δεν έχει πλέον καμία εσωτερική αντίφαση, το mailer env-var table αντικατοπτρίζει τον πραγματικό deployed κώδικα (discrete SMTP fields + webhook provider), και το self-host/SaaS MFA boundary είναι πλέον ρητά ξεκαθαρισμένο με cross-link.

Επόμενο run: item 4 της forty-second run λίστας (`75ee08d`, update-available notification για self-hosters, P40 — `docs/self-hosting.md`/`docs/updating.md`) · μετά items 5-9 της ίδιας λίστας (duplicate bill detection P46, backup mirror timestamp P48, suspended-workspace end-date email, platform AI key billing) · μετά νέο `git log --oneline <last-covered>..HEAD` σάρωμα για πιο πρόσφατα feat() commits (πολλά νέα μετά τα `12b755b`/`5b84f66` του τελευταίου git log που είδα).
