...
- **Find the entries still missing a rate.** Because a rate is never guessed, a foreign record saved without one keeps its printed number in the stored amount, which quietly joins your base-currency totals. **Reports** shows a **"N entries need an exchange rate"** panel above the figures it affects, listing every such record across expenses, income, receipts, items, subscriptions, statements and bills (largest printed amount first) with a link straight into the record. The panel only appears when multi-currency is on and something actually needs fixing.
- **Fix them without leaving Reports.** The panel groups the entries by printed currency (largest exposure first) and each group has a rate box: enter `1 USD = ? EUR` once and press **Apply to all N** to convert every USD record in one go, which is the normal case after a bank import created a batch of them. A single record can override the group with its own rate (a purchase from a different month) and be applied on its own; each row previews `printed → stored` before you commit it. Applying a rate converts that record's **whole** money side, exactly as its own edit form would: a receipt's net, VAT and every line price; an item's paid, current and target prices; a statement's minimum, paid and every charge; a subscription's first charge. Records that already have a rate are skipped rather than converted twice, so pressing Apply on a stale panel is harmless.
- **Optional "market rate" lookup (phase 2).** Every rate field (the six edit forms and the Reports panel) has a **Market rate** button that fetches the European Central Bank's reference rate for that currency and fills the field in. It is always a **suggestion you accept**: nothing is fetched on a schedule, nothing is applied on its own, and you can overwrite the number before saving (ECB reference rates are not what a card issuer actually charged, since issuers add a spread). Where a record has its own date (an expense, a receipt, a statement) the rate is looked up **as of that day**, and the button reports the day it really came from, because the ECB does not publish on weekends and holidays. The lookup carries only two currency codes and a date, never any record; it uses [Frankfurter](https://frankfurter.dev) (free, no API key, no account) and self-hosters who prefer not to call a third party can point `FX_RATE_API_URL` at their own instance. Nothing changes for a deployment that leaves multi-currency off.
