...
## UI Debt Queue
> Mobile UI consistency audit (re-verified 2026-06-30 βραδινό run, ui-auditor). Πόσο «universal» είναι το mobile UI σε σχέση με το web design system. Ranked: μικρά + P1 πρώτα. Το shared-theme item είναι foundation, άλλα εξαρτώνται από αυτό. Ο builder παίρνει το πρώτο TODO. Read-only audit, μηδέν app-code άλλαξε. **Re-audit: κανένα από τα 10 items δεν υλοποιήθηκε ακόμα από τον builder (το `theme.ts` παραμένει colors-only, το `ui.tsx` έχει μόνο Header/Spinner/Empty/ErrorText, `react-native-safe-area-context` ακόμα εκτός dependencies), οπότε όλη η ουρά μένει TODO· τα νούμερα ανανεώθηκαν.**
>
> Σύνοψη ευρημάτων (refreshed): **49** hardcoded hex εκτός `theme.ts` (40× `#000` text-on-accent + **8** alpha-tinted theme colors [`#ffd93d50`/`#ff475715`/`#ff475712`/`#00ff8815`/`#00ff8814`/`#00d4ff33`/`#00d4ff0a`/`#000000aa`] + 1 palette re-def `#f5f5f5`), το `theme.ts` έχει ΜΟΝΟ χρώματα (μηδέν spacing/radius/typography scale) + λείπουν `surface3` (#242424) / `orange` (#ffa502) που έχει το web (globals.css:11,22), μηδέν reusable Button/Card/Input/Badge/Chip primitives (το `input` style ξαναγράφεται σε **15** entries / 11 screens, `saveText`/`saveBtn`/`addBtn` **12×**, local `card`/`badge`/`chip` 13×), μηδέν light theme / theme context (το web έχει πλήρες light mode στο globals.css:37-50), `react-native-safe-area-context` **δεν είναι καν dependency** → `SafeAreaView` χωρίς insets (μηδέν bottom-inset, bottom-sheets κάτω από το home indicator), μόνο **2** `maxWidth` usages σε όλο το app (καμία στο content → edge-to-edge σε tablet/landscape), ~7 touch targets κάτω από 44pt (24×24 checkboxes ×6 + 30×30 lineDel + 36×36 rm + 40×36 backBtn). tsc καθαρό (read-only check, exit 0).
