...
**Τι δέχεται ο DS923 (10.0.1.5), μετρημένο από τη VM:** SMB **445/139 ανοιχτά**· SSH 22, rsync
...
**Προσάρτηση:** `//10.0.1.5/backups` σε `/mnt/nas-pharos`, cifs vers=3.0, εγγράψιμο από τον
...
δίσκοι του NAS. Μετρημένο μετά από 2,5 λεπτά χωρίς πρόσβαση: **0** TCP συνδέσεις προς το 10.0.1.5,
...
**(2) Το `ssh pharos` δεν περνάει πια authentication.** `ubuntu@10.0.1.11: Permission denied (publickey)`.
...
- **ΔΕΝ είναι η παγίδα της ανακυκλωμένης IP.** Ο server απαντά, το host key είναι
  ...
  αναγνωρίζει («is known and matches»). Είναι ο σωστός VM, το LAN 10.0.1.11.
- Το `~/.ssh/config` είναι διορθωμένο από τις 28/8: `Host pharos` προς `10.0.1.11`, `User ubuntu`,
