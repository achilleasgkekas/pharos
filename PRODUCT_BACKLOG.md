...
### P68. Επέκτασε το per-space tag (P34) σε Receipts/Subscriptions/Bills — 🚧 ΦΑΣΕΙΣ 1 (Receipts) + 2 (Subscriptions) SHIPPED 2026-09-07 (pharos-brain) — S — OSS, dogfooding-heavy
...
- **Φάση 2 (Subscriptions), shipped:** `Subscription.space` (ίδιο taxonomy, '' = χωρίς χώρο), picker στη φόρμα
  κρυμμένος όσο δεν υπάρχει ονομασμένος χώρος, αναζήτηση με το tag, `space` στο v1 API σε GET/POST/PATCH
  (trim + 40 χαρακτήρες, openapi + API.md), και νέος pure `lib/subscriptionSpaceCost.ts`.
  **Απόφαση που παρεκκλίνει από τη φάση 1, σκόπιμα:** οι συνδρομές ΔΕΝ μπαίνουν στο υπάρχον card «δαπάνες ανά
  χώρο» αλλά σε ΔΙΚΟ τους card («συνδρομές μηνιαία ανά χώρο»). Το gross card αθροίζει πραγματική δαπάνη
  (`Expense.amount`, `Receipt.total`)· η συνδρομή δεν έχει σύνολο, έχει ρυθμό, και ένα μηνιαίο ισοδύναμο
  ριγμένο σε ένα all-time άθροισμα δεν θα ήταν ημιτελές νούμερο, θα ήταν λάθος.
- **Μένει (φάση 3):** το ίδιο πεδίο σε `Bill`, και ο global space-filter στα money views
  (σήμερα φίλτρο ανά χώρο έχουν μόνο τα Expenses/Income· αποδείξεις και συνδρομές βρίσκονται με ελεύθερη
  αναζήτηση). Τα Bills είναι πραγματική δαπάνη, άρα εκεί ισχύει ξανά ο κανόνας της φάσης 1 (ίδιο card).
